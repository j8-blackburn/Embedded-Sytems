...
char sequence[NUMBER_OF_KEYS] = {'1', '9', '9', '8', '2'};
char reactivate[NUMBER_OF_KEYS] = {'1', '1', '1', '1', '1'};
char pressed[NUMBER_OF_KEYS] = {'0', '0', '0', '0', '0'};
int matrixKeyCode            = 0;
...
void LCDInterval(){
    LCDUpdate += TIME_INCREMENT_MS;
    if(LCDUpdate >= 60000){
        LCDUpdate = 0;
        displayCharPositionWrite(0, 0);
        if(systemHalted){
            displayStringWrite("System: HALTED  ");
            displayCharPositionWrite(0, 1);
            displayStringWrite("Enter 11111#    ");
        }else if(gasAlarm && tempAlarm){
            displayStringWrite("Alarm: GAS+TEMP ");
            displayCharPositionWrite(0, 1);
            displayStringWrite("Both Active!    ");
        }else if(gasAlarm){
            displayStringWrite("Alarm: Gas    ");
            displayCharPositionWrite(0, 1);
            displayStringWrite("Gas Detected!   ");
        }else if(tempAlarm){
            displayStringWrite("Alarm: OVER TEMP");
            displayCharPositionWrite(0, 1);
            char tempStr[20];
            sprintf(tempStr, "%.2fC Alarm    ", lm35TempC);
            displayStringWrite(tempStr);
        }else{
...
void keypadCodeUpdate(){
    if(incorrectCodes < 5){
        ...
        if(keyReleased != '\0' && keyReleased != '#'){
            ...
            char digitStr[17];
            for(int i = 0; i < NUMBER_OF_KEYS; i++){
                digitStr[i] = (i < matrixKeyCode) ? '*' : '-';
            }
            digitStr[NUMBER_OF_KEYS] = '\0';
            displayStringWrite(digitStr);
        }
        ...
        if(keyReleased == '#'){
            if(incorrectCodeLED){
                ...
                stringlength = sprintf(str, "Entered: %c%c%c%c%c\r\n", pressed[0], pressed[1], pressed[2], pressed[3], pressed[4]);
                ...
                if(systemHalted){
                    if(areEqualReactivate()){
                        ...
                        uartUsb.write(str, stringlength);
                        displayCharPositionWrite(0, 0);
                        displayStringWrite("System Active    ");
                        displayCharPositionWrite(0, 1);
                        displayStringWrite("Monitoring...    ");
                    } else {
                        incorrectCodeLED = 1;
                        incorrectCodes++;
                        displayCharPositionWrite(0, 0);
                        ...
                        displayCharPositionWrite(0, 1);
                        displayStringWrite("Press ## to try again    ");
                    }
                } else {
                    if(areEqual()){
                        systemHalted     = true;
                        ...
                        displayStringWrite("Wrong Code!    ");
                        displayCharPositionWrite(0, 1);
                        displayStringWrite("Press ## to retry    ");
                    }
                }
            }  
        }  
        ...
        displayCharPositionWrite(0, 1);
        displayStringWrite("Too many attampt");
    }
}
